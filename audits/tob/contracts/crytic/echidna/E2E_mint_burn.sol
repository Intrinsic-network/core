...
import '../../../../../contracts/IntrinsicPool.sol';
...
contract E2E_mint_burn {
    ...
    IntrinsicMinter minter;
    IntrinsicSwapper swapper;
    ...
    constructor() public {
        ...
        intrinsic = new SetupIntrinsic(token0, token1);

        minter = new IntrinsicMinter(token0, token1);
        swapper = new IntrinsicSwapper(token0, token1);
    ...
    function check_mint_invariants(
        int24 _tickLower,
        int24 _tickUpper,
        IntrinsicMinter.MinterStats memory bfre,
        IntrinsicMinter.MinterStats memory aftr
    ) internal {
        (, int24 currentTick, , , , , ) = pool.slot0();
    ...
    function _init(uint128 _seed) internal {
        ...
        // deploy the pool
        //
        intrinsic.createPool(poolParams.fee, poolParams.startPrice);
        pool = intrinsic.pool();
    ...
    function test_mint(uint128 _amount) public {
        ...
        (IntrinsicMinter.MinterStats memory bfre, IntrinsicMinter.MinterStats memory aftr) =
    ...
    function test_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            IntrinsicMinter.MinterStats memory bfre_burn,
            IntrinsicMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
