...
import { IntrinsicPoolSwapTest } from '../typechain/IntrinsicPoolSwapTest'
...
describe('IntrinsicPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('IntrinsicPoolSwapTest')
            const tester = (await testerFactory.deploy()) as IntrinsicPoolSwapTest
          ...
          let tester: IntrinsicPoolSwapTest
