...
import { TestIntrinsicSwapPay } from '../typechain/TestIntrinsicSwapPay'
...
import { TestIntrinsicReentrantCallee } from '../typechain/TestIntrinsicReentrantCallee'
...
describe('IntrinsicPool', () => {
  ...
  let factory: IntrinsicFactory
  let pool: MockTimeIntrinsicPool

  let swapTarget: TestIntrinsicCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeIntrinsicPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestIntrinsicReentrantCallee')
      ).deploy()) as TestIntrinsicReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestIntrinsicSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestIntrinsicSwapPay')
      underpay = (await underpayFactory.deploy()) as TestIntrinsicSwapPay
